...
namespace PointePayApp.Provider
{
    public class CategoryDataProvider : IDisposable
    {
        ...
        public bool DeleteAllSubCategoryOffline()
        {
            bool result = false;
            try
            {
                using (var db = new SQLite.SQLiteConnection(_dbPath))
                {
                    var _SubCategoryOffline = db.Query<PointePayApp.Model.SubCategoryOffline>("select * from SubCategoryOffline").ToList();
                    if (_SubCategoryOffline != null)
                    {
                        foreach (var itm in _SubCategoryOffline)
                        {
                            //Dlete row
                            db.RunInTransaction(() =>
                            {
        ...
        public List<SubCategoryOfflineViewModel> GetAllSubCategoryOfflineList()
        {
            List<SubCategoryOfflineViewModel> objList = new List<SubCategoryOfflineViewModel>();
            try
            {
                // Initialize the database if necessary
                using (var db = new SQLite.SQLiteConnection(_dbPath))
                {
                    objList = db.Query<PointePayApp.Model.SubCategoryOffline>("select * from SubCategoryOffline").Select(x => new SubCategoryOfflineViewModel
                    {
                        categoryId = x.categoryId,
                        organizationId = x.organizationId,
                        categoryCode = x.categoryCode,
            ...
            return objList;
        }//GetAllSubCustomerOfflineList

        public List<SubCategoryOfflineViewModel> GetsyncedSubCategoryOfflineList(string synced)
        {
            List<SubCategoryOfflineViewModel> objList = new List<SubCategoryOfflineViewModel>();
            try
            {
                ...
                using (var db = new SQLite.SQLiteConnection(_dbPath))
                {
                    objList = db.Query<PointePayApp.Model.SubCategoryOffline>("select * from SubCategoryOffline  Where synced='" + synced + "'").Select(x => new SubCategoryOfflineViewModel
                    {
                        categoryId = x.categoryId,
                        organizationId = x.organizationId,
                        categoryCode = x.categoryCode,
