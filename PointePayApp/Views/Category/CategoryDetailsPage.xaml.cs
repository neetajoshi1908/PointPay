                ...
                else
                {
                    if (ISOFile.FileExists("viewCategoryDetails"))//read current user login details    
                    {
                        using (IsolatedStorageFileStream fileStream = ISOFile.OpenFile("viewCategoryDetails", FileMode.Open))
                        {
                            ...
                            if (!string.IsNullOrEmpty(ObjCategoryData.fullImagePath))
                            {
                                imgCategory.ImageSource = new BitmapImage(new Uri(ObjCategoryData.fullImagePath, UriKind.RelativeOrAbsolute));
                            }
                            else 
                            {
                                imgCategory.ImageSource = new BitmapImage(new Uri("/Assets/category/category_icon.png", UriKind.RelativeOrAbsolute));
                            }
                        }
                    }
                }
