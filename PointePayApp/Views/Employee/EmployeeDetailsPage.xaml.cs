...
namespace PointePayApp.Views
{
    public partial class EmployeeDetailsPage : PhoneApplicationPage
    {
                ...
                else
                {
                    if (ISOFile.FileExists("viewEmployeeDetails"))//read current user login details    
                    {
                        using (IsolatedStorageFileStream fileStream = ISOFile.OpenFile("viewEmployeeDetails", FileMode.Open))
                        {
                            ...
                            if (!string.IsNullOrEmpty(ObjEmployeData.fullImagePath))
                            {
                                imgEmployee.ImageSource = new BitmapImage(new Uri(ObjEmployeData.fullImagePath, UriKind.RelativeOrAbsolute));
                            }
                            else
                            {
                                imgEmployee.ImageSource = new BitmapImage(new Uri("/Assets/Employee/account-circle.png", UriKind.RelativeOrAbsolute));
                            }
                        }
        ...
        {
            NavigationService.Navigate(new Uri("/Views/Employee/EmployeeListPage.xaml", UriKind.RelativeOrAbsolute));
        }
    }
}