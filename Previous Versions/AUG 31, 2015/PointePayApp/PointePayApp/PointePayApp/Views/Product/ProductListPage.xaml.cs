            ...
            try
            {
                    ...
                    if (rootObject.success == 1)
                    {
                        ...
                        foreach (var itm in rootObject.response.data)
                        {
                            ...
                            ListProductData.Add(new ProductViewModel { productId = itm.productId, code = itm.code, description = itm.description, currentPrice = itm.currentPrice, categoryId = itm.categoryId, organizationId = itm.organizationId, categoryCode = categoryCode, parentCategoryId=itm.parentCategoryId, parentCategoryCode = itm.parentCategoryCode, categoryDescription = itm.categoryDescription, active = itm.active, createDt = itm.createDt, lastModifiedDt = itm.lastModifiedDt, lastModifiedBy = itm.lastModifiedBy, Productdiscount = obj, fullImagePath = Source, upc = itm.upc, costPrice = itm.costPrice });
                        };
